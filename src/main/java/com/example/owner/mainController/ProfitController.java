...
public class ProfitController {
            ...
            switch (text) {
                case Constant.bugungi -> {
                    ...
                    myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                            "Sana: " + mapper.getCreatedDate() + "\n\n" +
                                    "\uD83D\uDCB5 Bugungi naxq qoldiqlar miqdori = " + ToDecimalUtil.toStr(mapper.getCash()) +
                                    " so'm\n"
                    ));
                ...
                case Constant.kun10 -> {
                    ...
                    for (ProfitMapper mapper : mappers) {
                        myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                                "Sana: " + mapper.getCreatedDate() + "\n" +
                                        "\uD83D\uDCB5 Umumiy Summa: " +  ToDecimalUtil.toStr(mapper.getCash()) + " so'm \n"));
                    }
                }
        ...
        if (residual.getStep().equals(Step.PLASTIK)) {
            switch (text) {
                case Constant.bugungi -> {
                    ...
                    myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                            "Sana: " + mapper.getCreatedDate() + "\n\n" +
                                    "\uD83D\uDCB5 Bugungi plastik qoldiqlar miqdori = " +  ToDecimalUtil.toStr(mapper.getCard()) +
                                    " so'm\n"
                    ));
                }
                ...
                case Constant.kun10 -> {
                    ...
                    for (ProfitMapper mapper : mappers) {
                        myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                                "Sana: " + mapper.getCreatedDate() + "\n" +
                                        "\uD83D\uDCB5 Umumiy Summa: " +  ToDecimalUtil.toStr(mapper.getCard()) + " so'm \n"));
                    }
                }
        ...
        if (residual.getStep().equals(Step.TOTALAMOUNTINPUTS)) {
            switch (text) {
                case Constant.bugungi -> {
                    ...
                    myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                            "Sana: " + mapper.getCreatedDate() + "\n\n" +
                                    "\uD83D\uDCB5 Bugungi umumiy qoldiqlar miqdori = " +  ToDecimalUtil.toStr(mapper.getTotal()) +
                                    " so'm\n"
                    ));
                }
                ...
                case Constant.kun10 -> {
                    ...
                    for (ProfitMapper mapper : mappers) {
                        myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                                "Sana: " + mapper.getCreatedDate() + "\n" +
                                        "\uD83D\uDCB5 Umumiy Summa: " + ToDecimalUtil.toStr(mapper.getTotal()) + " so'm \n"));
                    }
                }
    ...
    public boolean sendSummaByDateNaxd(Message message) {
        ...
        myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                "Sana " + mapper.getCreatedDate() + "\n\n" +
                        "\uD83D\uDCB5 Naxd qoldiqlar miqdori = " +  ToDecimalUtil.toStr(mapper.getCash()) +
                        " so'm\n"));
        return true;
    }
    ...
    public boolean sendSummaByDatePlastik(Message message) {
        ...
        myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                "Sana " + mapper.getCreatedDate() + "\n\n" +
                        "\uD83D\uDCB3 Plastik qoldiqlar miqdori = " +  ToDecimalUtil.toStr(mapper.getCard())+
                        " so'm\n"));
        return true;
    }
    ...
    public boolean sendSummaByDateTotal(Message message) {
        ...
        myTelegramBot.send(SendMsg.sendMsg(message.getChatId(),
                "Sana " + mapper.getCreatedDate() + "\n\n" +
                        "\uD83D\uDCB5 kundagi umimiy qoldiqlar miqdori = " +  ToDecimalUtil.toStr(mapper.getTotal()) +
                        " so'm\n"));
        return true;
    }
